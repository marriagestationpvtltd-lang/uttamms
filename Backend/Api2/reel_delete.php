...
$check = $asAdmin
    ? $pdo->prepare('SELECT id FROM user_reels WHERE id = ? AND status = "active" LIMIT 1')
    : $pdo->prepare('SELECT id FROM user_reels WHERE id = ? AND user_id = ? AND status = "active" LIMIT 1');
if ($asAdmin) {
...
$upd = $asAdmin
    ? $pdo->prepare('UPDATE user_reels SET status = "deleted", updated_at = NOW() WHERE id = ? LIMIT 1')
    : $pdo->prepare('UPDATE user_reels SET status = "deleted", updated_at = NOW() WHERE id = ? AND user_id = ? LIMIT 1');
if ($asAdmin) {
    $upd->execute([$reelId]);
} else {
    $upd->execute([$reelId, $userId]);
}
