...
if ($userId <= 0 || $storyId <= 0 || $privacy === '') {
    http_response_code(422);
    echo json_encode(['success' => false, 'message' => 'user_id, story_id and privacy are required']);
    exit;
}
...
$upd = $asAdmin
    ? $pdo->prepare('UPDATE user_stories SET privacy = ?, updated_at = NOW() WHERE id = ? LIMIT 1')
    : $pdo->prepare('UPDATE user_stories SET privacy = ?, updated_at = NOW() WHERE id = ? AND user_id = ? LIMIT 1');
if ($asAdmin) {
    $upd->execute([$privacy, $storyId]);
} else {
    $upd->execute([$privacy, $storyId, $userId]);
}
