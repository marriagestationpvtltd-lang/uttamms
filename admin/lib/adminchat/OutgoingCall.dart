...
class _CallScreenState extends State<CallScreen> with TickerProviderStateMixin {
  ...
  void _setupAudio() {
    ...
    _ringtonePlayerStateSubscription = _ringtonePlayer.onPlayerStateChanged
        .listen((state) {
          if (!mounted) return;
          // Keep looping ringback/ringtone while the call is waiting,
          // for both outgoing and incoming call UI.
  ...
  Future<void> _playRingtoneSingle() async {
    ...
    try {
      ...
      // Fallback: play default asset directly without Provider lookup
      try {
        await _ringtonePlayer.stop();
        await _ringtonePlayer.play(
          AssetSource('audio/outcall.mp3'),
          volume: 1.0,
        );
      } catch (_) {}
    }
  ...
  Future<void> _startCall() async {
    try {
      // Phase 1 – start waiting tone as soon as call UI opens.
      if (!widget.isOutgoingCall && mounted) {
      ...
      if (widget.isOutgoingCall) {
        ...
        // Fire FCM in parallel — the socket invite is the primary delivery path
        // for online users, and we don't want to block ringing on a slow/failing
        // FCM endpoint. Errors are swallowed (sendCallNotification returns bool).
        unawaited(
      ...
      // For incoming calls add a timeout so the overlay auto-dismisses if
      // the socket cancel/end event is missed (e.g. browser tab was backgrounded).
      if (!widget.isOutgoingCall) {
        _timeoutTimer = Timer(const Duration(seconds: 60), () async {
          if (!_callActive && !_ending) {
            await _endCall(notifyPeer: false);
          }
        });
      }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // ── Force-dismiss button (top-right) – clears stuck call UI
          if (!_callActive)
            Positioned(
              top: 48,
              right: 16,
              child: SafeArea(
                child: GestureDetector(
                  onTap: () => _endCall(notifyPeer: false),
                  child: Container(
  ...
  Widget _buildAvatar() {
    ...
    return AnimatedBuilder(
      ...
      builder: (_, child) {
        final scale = _callStatus == _CallStatus.connected
            ? 1.0
  ...
  Widget _buildSignalBars() {
    return AnimatedBuilder(
      ...
      builder: (_, __) {
        ...
        final active =
            (_signalController.value * _signalBarCount).floor() %
                _signalBarCount +
            1;
        return Row(
  ...
  Widget _buildParticipantsPanel() {
    ...
    final total =
        _conferenceParticipants.length + 2; // admin + original + added
    return AnimatedSize(
      duration: const Duration(milliseconds: 350),
