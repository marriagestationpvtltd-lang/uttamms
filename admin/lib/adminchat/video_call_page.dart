...
class _VideoCallScreenState extends State<VideoCallScreen> {
  ...
  final Map<int, Widget> _remoteVideoViews = {};
  ...
  void _setupAudioPlayer() {
    ...
    _ringtonePlayerStateSub = _ringtonePlayer.onPlayerStateChanged.listen((
      PlayerState state,
    ) {
  ...
  Future<void> _startCall() async {
    try {
      ...
      if (widget.isOutgoingCall) {
        ...
        // Fire FCM in parallel — don't block the socket invite on a slow/failing
        // FCM endpoint, otherwise the recipient never sees the incoming call UI.
        unawaited(
          NotificationService.sendVideoCallNotification(
        ...
        _callRejectedSubscription = _socketService.onCallRejected.listen((
          data,
        ) async {
          if (!mounted || _ending) return;
        ...
        _callAcceptedSubscription = _socketService.onCallAccepted.listen((
          data,
        ) async {
          if (!mounted || _ending) return;
          if (data['channelName']?.toString() == _channel) {
        ...
        _callBusySubscription = _socketService.onCallBusy.listen((data) async {
          if (!mounted || _ending) return;
          if (data['channelName']?.toString() == _channel) {
            setState(() {
              _recipientBusy = true;
      ...
      // Always listen for remote call teardown events (both outgoing and
      // incoming). This prevents stale/minimized calling UI when the caller
      // times out and cancels before admin answers.
      _callCancelledSubscription?.cancel();
      _callCancelledSubscription = _socketService.onCallCancelled.listen((
        data,
      ) async {
        if (!mounted || _ending) return;
        if (data['channelName']?.toString() == _channel) {
      ...
      _callEndedSubscription = _socketService.onCallEnded.listen((data) async {
        if (!mounted || _ending) return;
        if (data['channelName']?.toString() == _channel) {
          await _endCall(notifyPeer: false);
        }
      ...
      _participantAcceptedSubscription = _socketService
      ...
      _participantRejectedSubscription = _socketService
          ...
          .listen((data) {
            ...
            if (data['channelName']?.toString() == _channel) {
              ...
              Future.delayed(const Duration(seconds: 4), () {
                if (mounted) {
                  setState(() {
                    _conferenceParticipants.removeWhere(
                      (p) =>
      ...
      _engine.registerEventHandler(
        RtcEngineEventHandler(
          ...
          onUserOffline: (_, uid, __) {
            ...
            if (_remoteUid == uid) {
              _remoteUid = _remoteUids.isEmpty ? null : _remoteUids.first;
            }
  ...
  void _setupRemoteVideo(int uid) {
    ...
    setState(() {
      _remoteVideoViews[uid] = videoSurface;
    });
  }
  ...
  Future<void> _endCall({bool notifyPeer = true}) async {
    ...
    final String callStatus = _recipientBusy
        ? 'busy'
        : (_callActive ? 'answered' : 'missed');
  ...
  Widget build(BuildContext context) {
    ...
    final double localWidth = (size.width * _kLocalPreviewWidthRatio)
        ...
        .toDouble();
    final double localHeight = (localWidth * _kLocalPreviewAspectRatio)
  ...
  Widget _buildRemoteVideoStage() {
    if (_remoteVideoViews.isEmpty) {
      return _buildRemotePlaceholder();
    }

    if (_remoteVideoViews.length == 1) {
      return _remoteVideoViews.values.first;
    }

    final entries = _remoteVideoViews.entries.toList();
    final int columns = entries.length == 2 ? 2 : 2;

    return GridView.builder(
      physics: const NeverScrollableScrollPhysics(),
      padding: const EdgeInsets.fromLTRB(10, 76, 10, 130),
      itemCount: entries.length,
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: columns,
        crossAxisSpacing: 8,
        mainAxisSpacing: 8,
        childAspectRatio: entries.length <= 2 ? 0.78 : 0.9,
      ),
      itemBuilder: (_, index) {
        final entry = entries[index];
        return ClipRRect(
          borderRadius: BorderRadius.circular(14),
          child: Container(color: _kSlateDark, child: entry.value),
        );
      },
    );
  }
  ...
  Widget _buildTopBar() {
    ...
    final Color statusColor = _callStatus == _VCallStatus.connected
        ? _kEmerald
        : (_callStatus == _VCallStatus.busy ? Colors.orange : _kAmber);
      ...
      builder: (context, constraints) {
        ...
        final List<Widget> buttons = [
          ...
          _controlButton(
            ...
            onPressed: (_videoEnabled && controlsEnabled)
                ? _switchCamera
                : null,
            size: buttonSize,
