...
class _PartnerPreferencesPageState extends State<PartnerPreferencesPage> {
  ...
  List<String> _ageOptions = List.generate(
    (_defaultMaximumPartnerAge - PartnerAgePreferenceBounds.minimumAllowedAge) +
        1,
    (index) =>
        (PartnerAgePreferenceBounds.minimumAllowedAge + index).toString(),
  );
  ...
  // Height options from shared constants
  List<String> get _heightOptions => kHeightOptions;
  ...
  // Dynamic options — loaded from master API
  List<String> _educationOptions = ['Any'];
  List<String> _occupationOptions = ['Any'];
  ...
  void initState() {
    ...
    // Height options come from kHeightOptions — no generation needed
    _loadOptions();
    _loadInitialData();
  }
  ...
  Future<void> _loadOptions() async {
    ...
    if (!mounted) return;
    setState(() {
      _educationOptions = ['Any', ...?(all['degree'])];
      _occupationOptions = ['Any', ...?(all['occupationtype'])];
    });
  }
  ...
  bool _validateForm() {
    setState(() {
      _fieldErrors = {
        'age': (_minAge == null || _maxAge == null)
            ? 'Please select age range'
            : null,
        'height': (_minHeight == null || _maxHeight == null)
            ? 'Please select height range'
            : null,
        'maritalStatus': _selectedMaritalStatus.isEmpty
            ? 'Please select at least one option'
            : null,
        'religion': _selectedReligion.isEmpty
            ? 'Please select at least one option'
            : null,
      };
  ...
  Future<void> _loadStatesForSelectedCountries(
      {List<String>? preferredSelection}) async {
    ...
    try {
      ...
      final selected =
          _normalizeAnySelection(preferredSelection ?? _selectedState)
  ...
  Future<void> _loadDistrictsForSelectedStates(
      {List<String>? preferredSelection}) async {
    ...
    try {
      ...
      final selected =
          _normalizeAnySelection(preferredSelection ?? _selectedDistrict)
  ...
  Future<void> _loadSavedPreferences() async {
    ...
    final result = await UserPartnerPreferenceService()
        .fetchPartnerPreference(userId: userId);
    ...
    setState(() {
      ...
      _selectedCommunity =
          _parsePreferenceList(data['caste'] ?? data['community']);
      ...
      _selectedDistrict =
          _parsePreferenceList(data['city'] ?? data['district']);
      _selectedEducation =
          _parsePreferenceList(data['qualification'] ?? data['education']);
      _selectedOccupation =
          _parsePreferenceList(data['profession'] ?? data['proffession']);
    ...
    await _loadStatesForSelectedCountries(
        preferredSelection: List<String>.from(_selectedState));
    if (!mounted) return;
    setState(() {
      _selectedState = _resolveLocationSelection(_selectedState, _stateMap);
    });
    await _loadDistrictsForSelectedStates(
        preferredSelection: List<String>.from(_selectedDistrict));
    if (!mounted) return;
    setState(() {
      _selectedDistrict =
          _resolveLocationSelection(_selectedDistrict, _districtMap);
    });
  }
  ...
  Future<void> _validateAndSubmit() async {
    ...
    try {
      ...
      final result = await service.savePartnerPreference(
        ...
        cityIds: districtIds,
        community:
            _selectedCommunity.isNotEmpty ? _selectedCommunity.join(',') : null,
        motherTongue: _selectedMotherTongue.isNotEmpty
            ? _selectedMotherTongue.join(',')
            : null,
        country:
            _selectedCountry.isNotEmpty ? _selectedCountry.join(',') : null,
        state: _selectedState.isNotEmpty ? _selectedState.join(',') : null,
        district:
            _selectedDistrict.isNotEmpty ? _selectedDistrict.join(',') : null,
        education:
            _selectedEducation.isNotEmpty ? _selectedEducation.join(',') : null,
        occupation: _selectedOccupation.isNotEmpty
            ? _selectedOccupation.join(',')
            : null,
  ...
  void _generateExpectations() {
    ...
    if (_minAge != null && _maxAge != null) {
      expectations.add(
          'Looking for a partner aged between $_minAge and $_maxAge years.');
    }
    ...
    if (_selectedMaritalStatus.isNotEmpty &&
        !_selectedMaritalStatus.contains('Any')) {
      expectations.add(
    ...
    if (_selectedCommunity.isNotEmpty && !_selectedCommunity.contains('Any')) {
      expectations
          .add('Community/caste preference: ${_selectedCommunity.join(", ")}.');
    }

    if (_selectedMotherTongue.isNotEmpty &&
        !_selectedMotherTongue.contains('Any')) {
      expectations.add(
    ...
    if (_selectedEducation.isNotEmpty && !_selectedEducation.contains('Any')) {
      expectations
          .add('Educational qualification: ${_selectedEducation.join(", ")}.');
    }

    if (_selectedOccupation.isNotEmpty &&
        !_selectedOccupation.contains('Any')) {
      expectations
          .add('Profession preference: ${_selectedOccupation.join(", ")}.');
    }
  ...
  Widget _buildSingleSelectField({
    ...
    final String? validatedValue =
        (selectedItem != null && options.contains(selectedItem))
            ? selectedItem
            : null;
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: RegistrationStepContainer(
          onContinue: (_isSubmitting || _isLoadingInitialData)
              ? null
              : _validateAndSubmit,
          ...
          child: Column(
            ...
            children: [
              ...
              RegistrationStepHeader(
                ...
                subtitle:
                    'Help us understand your ideal life partner. Your preferences help us find better matches.',
                currentStep: 10,
                totalSteps: 11,
                onBack: () => Navigator.pop(context),
                    ...
                    child: TypingDropdown<String>(
                      ...
                      onChanged: (value) {
                        setState(() {
                          ...
                          if (_maxAge != null &&
                              int.parse(_maxAge!) < int.parse(value!)) {
                            _maxAge = value;
                          }
                    ...
                    child: TypingDropdown<String>(
                      ...
                      onChanged: (value) {
                        setState(() {
                          ...
                          if (_minAge != null &&
                              int.parse(_minAge!) > int.parse(value!)) {
                            _minAge = value;
                          }
                          if (_hasValidationErrors) {
                            _fieldErrors['age'] = null;
                ...
                children: [
                  Expanded(
                    child: TypingDropdown<String>(
                      ...
                      showError:
                          _fieldErrors['height'] != null && _minHeight == null,
                      ...
                      onChanged: (value) {
                        setState(() {
                          ...
                          if (_maxHeight != null) {
                            ...
                            final maxCm =
                                int.parse(_maxHeight!.split(' ').first);
                            if (maxCm < minCm) {
                              _maxHeight = value;
                            }
                          }
                  ...
                  Expanded(
                    child: TypingDropdown<String>(
                      ...
                      showError:
                          _fieldErrors['height'] != null && _maxHeight == null,
                      prefixIcon: Icons.height,
                      onChanged: (value) {
                        setState(() {
                          _maxHeight = value;
                          ...
                          if (_minHeight != null) {
                            final minCm =
                                int.parse(_minHeight!.split(' ').first);
                    ...
                    onConfirm: (selected) {
                      setState(() {
                        _selectedMaritalStatus =
                            _normalizeAnySelection(selected);
                        if (_hasValidationErrors) {
                          _fieldErrors['maritalStatus'] =
              ...
              _buildMultiSelectField(
                ...
                onTap: () {
                  _showMultiSelectDialog(
                    ...
                    icon: Icons.language,
                    onConfirm: (selected) {
                      setState(() => _selectedMotherTongue =
                          _normalizeAnySelection(selected));
                    },
                  );
                },
              ...
              SectionHeader(
                ...
                subtitle:
                    'Diet, smoking and drinking habits preferences (Optional)',
              ...
              _buildMultiSelectField(
                ...
                onTap: () {
                  _showMultiSelectDialog(
                    ...
                    onConfirm: (selected) {
                      setState(() =>
                          _selectedDiet = _normalizeAnySelection(selected));
                    },
                  );
                },
              ),
              ...
              SectionHeader(
                ...
                subtitle:
                    'Any additional preferences or expectations (Optional)',
              ...
              _buildTextAreaField(
                ...
                hint:
                    'E.g. looking for someone who values family, enjoys travel...',
